...
export async function navigateToShippingZone() {
		...
		const zone = await page.$x( `//td/a[contains(text(),"${testZoneName}")]` );
		if ( ! zone || zone.length === 0 ) {
			await clickAndWait( '.wc-shipping-zones-heading > .page-title-action' )
			await page.type( '#zone_name',  testZoneName )
			await Promise.all([ page.waitForSelector( '.wc-backbone-modal-header' ), page.click( '.wc-shipping-zone-add-method' ) ])
			await Promise.all([ page.waitForSelector('.wc-shipping-zone-method-title > .wc-shipping-zone-method-settings'), page.click( '.wc-backbone-modal #btn-ok' ) ] )
		} else {
...
export async function waitForShippingZoneMethods() {
	await page.waitForSelector( '.wc-shipping-zone-method-title > .wc-shipping-zone-method-settings' );
}
